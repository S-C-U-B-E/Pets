...
public class PetProvider extends ContentProvider {
    ...
    private Uri insertPet(Uri uri, ContentValues values) {
        ...
        String name = values.getAsString(PetsContract.PetsEntry.COLUMN_PETS_NAME);
        if (name == null) {
            throw new IllegalArgumentException("Pet requires a name");
        }

        // Check that the gender is valid
        Integer gender = values.getAsInteger(PetsContract.PetsEntry.COLUMN_PETS_GENDER);
        if (gender == null || !PetsContract.PetsEntry.isValidGender(gender)) {
            throw new IllegalArgumentException("Pet requires valid gender");
        }

        // If the weight is provided, check that it's greater than or equal to 0 kg
        Integer weight = values.getAsInteger(PetsContract.PetsEntry.COLUMN_PETS_WEIGHT);
    ...
    public String getType(Uri uri) {
        return "type will be sent from here";
    }
}
